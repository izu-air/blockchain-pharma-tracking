    ...
    <div className="panel min-w-0">
      ...
      {verification?.expired && (
        <div className="mb-4 rounded-xl border border-amber-500/40 bg-amber-950/30 p-3 text-sm font-medium text-amber-100">
          <p className="break-words leading-snug">
            Срок годности продукта истёк. Покупателю следует отказаться от использования.
          </p>
        </div>
      )}
      <div className="mb-4 flex flex-col gap-2 border-b border-white/10 pb-4 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          ...
          <p className="break-all text-sm text-slate-400">
            Serial number: <span className="font-mono">{product.serialNumber}</span>
          </p>
        ...
        <span className={`w-fit shrink-0 rounded-md px-3 py-1 text-sm font-medium ${statusClasses[product.status]}`}>
          {statusLabels[product.status]}
        </span>
      </div>

      <div className="grid gap-3 text-sm text-slate-200 md:grid-cols-2">
        <Info label="Производитель"        value={formatAddress(product.manufacturer)} mono />
        <Info label="Текущий владелец"     value={formatAddress(product.currentOwner)} mono />
        ...
        {batch && (
          <>
            ...
            <Info label="Hash метаданных"       value={`${batch.metadataHash.slice(0, 10)}…`} mono />
          </>
        )}
        {metadata && (
          <>
      ...
      {metadata?.description && (
        <p className="mt-4 break-words text-sm text-slate-300">{metadata.description}</p>
      )}
    </div>
  );
}
...
function Info({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="min-w-0">
      ...
      <p className={`break-words font-medium text-slate-100 ${mono ? "font-mono text-sm" : ""}`}>
        {value}
      </p>
    </div>
  );
